    ...
    <div id="form-view" class="mb-5">
      ...
      <form>
        <div id="form-content" class="p-5">
          <div class="controll">
            <p class="q-title has-text-info">-性別-</p>
            <input
              v-model="sexuality"
              type="radio"
              name="sexuality"
          ...
          <div class="controll">
            <p class="q-title has-text-info">-生年月日-</p>
            <div class="is-flex">
              <div class="is-flex">
                <div class="select">
...
export default {
  ...
  methods: {
    checkStep1() {
      if(this.$store.getters.sexuality && this.$store.getters.birthYear && this.$store.getters.birthMonth && this.$store.getters.birthDay) {
        this.buttonActive();
      } else {
        this.buttonInactive();
      }
    ...
    yearsGenerator() {
      ...
      for (let y = 1920; y <= 2000; y++){
        if (y > 1988) {
          years.push(`${y} (平成${y-1988}年)`);
        } else if (y > 1925) {
          years.push(`${y} (昭和${y-1925}年)`);
        } else if (y > 1911) {
          years.push(`${y} (大正${y-1911}年)`);
        }
      }
      return years;
    },
